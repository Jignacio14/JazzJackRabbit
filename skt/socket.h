...
public:
     ...
     *
     * Ambos lo hacen byte a byte, por eso bytewise, es decir, no interesa el orden de los
     * mismos porque son "paquetes individuales"
     *
    ...
    /*
     * `Socket::sendall_bytewise` envía exactamente `sz` bytes leídos del buffer, ni más,
     * ni menos. `Socket::recvall_bytewise` recibe exactamente sz bytes.
     * Ambos lo hacen byte a byte, por eso bytewise, es decir, no interesa el orden de los
     * mismos porque son "paquetes individuales"
     *
     * Si hay un error se lanza una excepción.
    ...
    /*
     *  Equivalentes de Socket::sendall_bytewise y Socket::recvall_bytewise pero para enviar
     *  exactamente una halfword (2 bytes) donde sí importa el orden de los bytes debido
     *  al endianness
     * */
